...
// On torch >= 2.10 the pybind11 surface ships in a separate, test-only
// artifact; ``-DTORCH_LEGACY_LIB`` selects the matching module name so it
// matches the .so/.dll filename that ``importlib.import_module`` resolves.

#ifdef _WIN32

#ifdef TORCH_LEGACY_LIB
#ifdef USE_CUDA
#define LIBRARY_FILE_NAME custom_ops_torch_legacy_gpu
#else
#define LIBRARY_FILE_NAME custom_ops_torch_legacy
#endif
#else
#ifdef USE_CUDA
...
#ifdef TORCH_LEGACY_LIB
#ifdef USE_CUDA
#define LIBRARY_FILE_NAME libcustom_ops_torch_legacy_gpu
#else
#define LIBRARY_FILE_NAME libcustom_ops_torch_legacy
#endif
#else
#ifdef USE_CUDA
