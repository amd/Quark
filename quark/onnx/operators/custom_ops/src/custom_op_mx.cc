...
Ort::Value MXFixNeuronKernel::cast_to_fp32(
...
Ort::Value MXFixNeuronKernel::cast_to_fp16(
  OrtKernelContext* context, Ort::Value& input
) {
...
void MXFixNeuronKernel::Compute(OrtKernelContext* context) {
  ...
  if (input_tensor.GetTensorTypeAndShapeInfo().GetElementCount() == 1) {
    ...
    if ((axis_ >= -tensor_rank && axis_ <= tensor_rank - 1) == false)
      ORT_CXX_API_THROW(
        "The axis of MX should be in the valid range.",
    ...
    auto sliced_tensor = slice(context, mx_tensor, dimensions[axis_no_neg]);
