    ...
    def get_bias_corr_pattern_augment_graph(self) -> tuple[dict[str, tuple[str, str, str | None]], set[str]]:
        ...
        for node in model.graph.node:
            ...
            if node.op_type in self.target_type:
                ...
                all_sub_model_in_out.extend([sub_model_input, sub_model_output])
        for inter_output in set(all_sub_model_in_out):
...
def bias_correction(
    fmodel_input: str | Path | onnx.ModelProto,
    qmodel_input: str | Path | onnx.ModelProto,
    use_external_data_format: bool,
    calibration_data_reader: CachedDataReader,
    activation_type: QuantType | ExtendedQuantType,
    calibrate_method: PowerOfTwoMethod | CalibrationMethod,
    ...
    for bc_node_name, bc_in_out_bias_tensor in quant_bc_pattern.items():
        if bc_node_name in float_node_obj_map:
            ...
            bc_input_tensor_name = bc_in_out_bias_tensor[0]
            bc_output_tensor_name = bc_in_out_bias_tensor[1]
            quant_node_bias_name = bc_in_out_bias_tensor[2]
            quant_in_tensor = quant_clean_merged_dict[bc_input_tensor_name]
            ...
            if quant_out_tensor.ndim == 5:
                ...
                float_quant_diff_mean = np.mean((float_out_tensor - quant_out_tensor), axis=axis_reduce_4dim)
            elif quant_out_tensor.ndim == 3:
                axis_reduce_2dim = (0, 1)
                float_quant_diff_mean = np.mean((float_out_tensor - quant_out_tensor), axis=axis_reduce_2dim)
            else:
            ...
            if quant_node_bias_name:
                ...
                if calibrate_method in [PowerOfTwoMethod.NonOverflow, PowerOfTwoMethod.MinMSE]:
                    ...
                    *_, quantized_data = quantize_data(
