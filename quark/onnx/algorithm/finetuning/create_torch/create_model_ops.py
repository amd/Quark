...
def extract_weight_and_bias(params: list[Any]) -> tuple[NDArray[Any], NDArray[Any] | None]:
...
def load_weight_and_bias(layer: nn.Module, weight: NDArray[Any], bias: NDArray[Any] | None) -> None:
...
) -> tuple[QuantizeWrapper, QuantizeWrapper | None]:
...
def convert_matmul(node: onnx.NodeProto, layer_params: list[Any], layer_qinfos: list[Any]) -> tuple[QMatMul, None]:
    ...
    layer: QMatMul | type[QMatMul] = QMatMul
...
def convert_norm(
    node: onnx.NodeProto, layer_params: list[Any], layer_qinfos: list[Any]
) -> tuple[QInstanceNorm2d | QLayerNorm, None]:
    ...
    layer: QInstanceNorm2d | type[QInstanceNorm2d] | QLayerNorm | type[QLayerNorm] = (
...
def convert_act(node: onnx.NodeProto) -> nn.Module | None:
    ...
    if node is None:
        ...
        return None
    elif node.op_type not in ActivationMapping:
