...
class ConvQDQToQOPTransform(Transform):
    ...
    def replacement(self, match_node: NodeTree) -> Any:
        ...
        w_zero_point_node = match_node.input_nodes[0].input_nodes[1].input_nodes[2]
        ...
        rescale_b_value = ((b_value * b_scale_value + b_zero_point_value) - w_zero_point_value) / w_scale_value
        rescale_b = model_utils.generate_initializer(rescale_b_value, dtype=np.int32, name=b_node.node.name)
        rescale_b_node = NodeTree(rescale_b, weights=None, input_nodes=[], metadata=None)
