...
def split_config_levels(input_config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if "extra_options" in raw_config:
        if "FastFinetune" in raw_config["extra_options"]:
...
class SearchSpace:
    ...
    def __init__(self, conf: dict[str, Any], logger: logging.Logger | None = None) -> None:
        ...
        if len(self.level1_space_config) > 0:
            for level1_key in self.level1_space_config:
        ...
        if len(self.level2_space_config) > 0:
            for level2_key in self.level2_space_config:
        ...
        if len(self.level3_space_config) > 0:
            for level3_key in self.level3_space_config:
    ...
    def verify_one_config_base(
        ...
        valid_flag = True
        if base_item not in base_config or base_item in base_config and base_config[base_item] != verify_standard:
            valid_flag = False
    ...
    def verify_one_config(
        ...
        if verify_item in verify_config:
            if base_item not in base_config or base_item in base_config and base_config[base_item] != verify_standard:
                self.logger.info(f"Please set {base_item} to be {verify_standard}, when you use {verify_item}! ")
...
class AutoSearch:
    ...
    def build_all_configs(self, search_space_config: dict[str, Any]) -> list[Any]:
        # TODO support crypto_mode
        if "crypto_mode" in search_space_config:
    ...
    def runner(self, one_config: dict[str, Any]) -> float:
        ...
        # set level3 config
        if len(level3_keys) != 0:
            if "FastFinetune" not in self.config.global_quant_config.extra_options:
    ...
    def objective(self, trial: Any, quant_one_time_param: list[Any] = [0, -1]) -> float:
        ...
        if stage_name not in self.config.global_quant_config.extra_options:
    ...
    def process_joint_param(self, all_configs: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item_config in all_configs:
            *_, l3_conf = split_config_levels(item_config)
            if numiter_lr_key in l3_conf:
    ...
    def search_model(
        ...
        if "find_n_candidates" in self.auto_search_config.search_stop_condition:
            find_n_candidates = self.auto_search_config.search_stop_condition["find_n_candidates"]
        else:
            find_n_candidates = -1
        if "iteration_limit" in self.auto_search_config.search_stop_condition:
            iteration_limit = self.auto_search_config.search_stop_condition["iteration_limit"]
        else:
            iteration_limit = -1
        if "time_limit" in self.auto_search_config.search_stop_condition:
