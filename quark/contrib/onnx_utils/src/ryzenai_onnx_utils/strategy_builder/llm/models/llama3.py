...
@register_model
class Llama3_8B(LlmModelBase):
    model_type = LlmModel.LLAMA3_8B

    def _override_prefill_fusion_attributes(self):
        # Prefill fusion for llama3 has to be split in two to avoid BO size
    ...
    def matches(cls, input_path: str) -> bool:
        return re.findall(r"llama[-_]?3[\.\d]*[-_]?8b", input_path)
