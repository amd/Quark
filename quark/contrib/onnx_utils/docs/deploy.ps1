...
Invoke-SafeCommand { cd html } "cd down" $true
Invoke-SafeCommand { git add . } "Add new files" $false
Invoke-SafeCommand { git commit -m "Update docs" } "Commit new files" $false
Invoke-SafeCommand { git push origin gh-pages } "git push" $false
