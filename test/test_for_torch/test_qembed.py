...
from quark.torch.quantization import Config, QuantizationConfig, Uint4PerChannelSpec, Int8PerTensorSpec, Int8PerChannelSpec, Uint8PerTensorSpec, Uint4PerTensorSpec
...
def test_net():
    ...
    for para in params:
        ...
        INT8_PER_CHANNEL_SPEC = Int8PerChannelSpec(
                ...
                ch_axis=0,
                is_dynamic=False).to_quantization_spec()
        quant_config = QuantizationConfig(input_tensors=observal_val, weight=INT8_PER_CHANNEL_SPEC, output_tensors=observal_val, bias=observal_val)
        ...
        layer_type_quant_config = {torch.nn.modules.sparse.EmbeddingBag: QuantizationConfig(weight=INT4_PER_TENSOR_SPEC), torch.nn.modules.sparse.Embedding: QuantizationConfig(weight=INT4_PER_TENSOR_SPEC)}
        ...
        assert(
            hasattr(quant_model.embedding_bag, "_weight_quantizer")
        )
        ...
        assert (
            hasattr(quant_model.embedding, "_weight_quantizer")
        )
        quantized_model = quantizer.freeze(quant_model.eval())
