...
input_tensor = torch.tensor([[0.6191, 0.7309, 0.3578, 0.8316, 0.5851, 0.2182, 0.9903, 0.2811, 0.0726, 0.3206],
...
def process_int_per_tensor_qdq(quantization_spec, device, scale, zero_point):
    ...
    realq_res = quark.torch.kernel.scaled_real_quantize(  # type: ignore[attr-defined]
        quantization_spec.dtype.value, input_tensor, scale.to(device), zero_point.to(device), quantization_spec.ch_axis,
        quantization_spec.group_size, -128, 127, getattr(quantization_spec.round_method, "value", None),
        getattr(quantization_spec.qscheme, "value", None))
    deq_res = quark.torch.kernel.dequantize(quantization_spec.dtype.value, realq_res, scale.to(device),
                                            zero_point.to(device), quantization_spec.ch_axis,
    ...
    golden_tensor = torch.fake_quantize_per_tensor_affine(  # type: ignore[attr-defined]
        input_tensor, scale.to(device),
        zero_point.to(device).to(torch.int), -128, 127)
...
def test_int_per_tensor_quantize_2():
    ...
    tensor_qdq_ref = quark.torch.kernel.scaled_fake_quantize(dtype, tensor, scale, zero_point.to(torch.int), ch_axis, group_size, quant_min, quant_max, round_method, qscheme, 'None')

    tensor_q = quark.torch.kernel.scaled_real_quantize(
        dtype, tensor, scale, zero_point, ch_axis, group_size, quant_min, quant_max,
        round_method, qscheme)
    tensor_qdq = quark.torch.kernel.dequantize(
            dtype, tensor_q, scale, zero_point, ch_axis,
            group_size, qscheme).to(torch.float32)
...
def process_int_per_channel_qdq(quantization_spec, device, scale, zero_point):
    ...
    golden_tensor = torch.fake_quantize_per_channel_affine(  # type: ignore[attr-defined]
        input_tensor, scale.to(device),
        zero_point.to(device).to(torch.int), 1, -128, 127)
    assert torch.equal(deq_res, golden_tensor)



def test_int_per_channel_quantize():
    DEFAULT_INT8_PER_CHANNEL_SYM_SPEC = QuantizationSpec(dtype=Dtype.int8,
...
def process_int_per_group_qdq(quantization_spec, device, scale, zero_point):
    ...
    realq_res = quark.torch.kernel.scaled_real_quantize(  # type: ignore[attr-defined]
        quantization_spec.dtype.value, input_tensor, scale.to(device), zero_point.to(device), quantization_spec.ch_axis,
        quantization_spec.group_size, -8, 7, getattr(quantization_spec.round_method, "value", None),
        getattr(quantization_spec.qscheme, "value", None))
    deq_res = quark.torch.kernel.dequantize(quantization_spec.dtype.value, realq_res, scale.to(device),
                                            zero_point.to(device), quantization_spec.ch_axis,
                                            quantization_spec.group_size,
    ...
    golden_tensor = torch.fake_quantize_per_channel_affine(  # type: ignore[attr-defined]
        ...
        zero_point.to(device).to(torch.int).view(-1), 0, -8, 7)
...
def test_int_per_group_quantize():
    DEFAULT_INT8_PER_GROUP_SYM_SPEC = QuantizationSpec(dtype=Dtype.int4,
    ...
    scale = torch.tensor([[0.0033, 0.0038], [0.0039, 0.0039], [0.0018, 0.0035], [0.0033, 0.0039], [0.0036, 0.0028],
                          [0.0035, 0.0033], [0.0039, 0.0031], [0.0034, 0.0038], [0.0037, 0.0035], [0.0037, 0.0032]])
...
def process_fp8_per_tensor_qdq(quantization_spec, device, scale, max_norm):
    ...
    quark_tensor = quark.torch.kernel.scaled_fake_quantize(  # type: ignore[attr-defined]
        quantization_spec.dtype.value, input_tensor, scale.to(device), None, quantization_spec.ch_axis,
        quantization_spec.group_size, -max_norm, max_norm, getattr(quantization_spec.round_method, "value", None),
        getattr(quantization_spec.qscheme, "value", None), None)

    realq_res = quark.torch.kernel.scaled_real_quantize(  # type: ignore[attr-defined]
        quantization_spec.dtype.value, input_tensor, scale.to(device), None, quantization_spec.ch_axis,
        quantization_spec.group_size, -max_norm, max_norm, getattr(quantization_spec.round_method, "value", None),
        getattr(quantization_spec.qscheme, "value", None))
    deq_res = quark.torch.kernel.dequantize(quantization_spec.dtype.value, realq_res, scale.to(device), None,
                                            quantization_spec.ch_axis, quantization_spec.group_size,
                                            getattr(quantization_spec.qscheme, "value", None))

    assert torch.equal(deq_res, quark_tensor)


@pytest.mark.parametrize("dtype,max_norm", [
    (Dtype.fp8_e4m3, 448),
    (Dtype.fp8_e5m2, 57344),
])
def test_fp8_per_tensor_quantize(dtype, max_norm):
    DEFAULT_FP8_PER_TENSOR_SYM_SPEC = QuantizationSpec(dtype=dtype,
                                                       qscheme=QSchemeType.per_tensor,
                                                       observer_cls=PerTensorMinMaxObserver,
                                                       symmetric=True,
                                                       scale_type=ScaleType.float,
...
def test_fp8_per_channel_quantize(dtype, max_norm):
    DEFAULT_FP8_PER_CHANNEL_SYM_SPEC = QuantizationSpec(dtype=dtype,
                                                        qscheme=QSchemeType.per_channel,
                                                        observer_cls=PerChannelMinMaxObserver,
                                                        symmetric=True,
                                                        scale_type=ScaleType.float,
...
def test_fp16_per_channel_quantize_axis_range(dtype, max_norm):
    DEFAULT_FP16_PER_CHANNEL_SYM_SPEC = QuantizationSpec(dtype=dtype,
                                                         qscheme=QSchemeType.per_channel,
                                                         observer_cls=PerChannelMinMaxObserver,
                                                         symmetric=True,
                                                         scale_type=ScaleType.float,
