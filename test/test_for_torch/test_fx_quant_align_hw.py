...
from quark.torch.quantization.nn.modules import QuantLeakyReLU, QuantLinear, QuantConv2d, QuantConvTranspose2d, \
    QuantAdaptiveAvgPool2d, QuantAvgPool2d, QuantizedConvBatchNorm2d, QuantConvTransposeBatchNorm2d
...
from quark.torch.quantization.graph.torch_utils import is_avg_pool2d_node, is_clip_node, is_relu_act_node, is_mean_node, \
    is_batchnorm_node, is_adaptive_avg_pool2d_node, is_split_node, is_slice_node, \
    _is_split_with_size_node, _is_sample_split_node, is_sigmoid_node, is_hardsigmoid_node, \
    is_silu_node, is_hardswish_node, is_mul_node, QUANT_CONV_WITH_BN
...
INT8_PER_TENSOR_SPEC = QuantizationSpec(dtype=Dtype.int8,
                                        ...
                                        is_dynamic=False)
quant_tensor_config = QuantizationConfig(input_tensors=INT8_PER_TENSOR_SPEC,
...
def test_use_over_once_module_optim():
    ...
    for each_fp_model in [float_model1, float_model2, float_model3, float_model4, float_model5]:
        ...
        graph_model_1 = capture_pre_autograd_graph(each_fp_model, example_inputs)
        for each_graph in [graph_model_0, graph_model_1]:
            graph_model = trans_opsfunc_2_quant_module(each_graph)
            ...
            for module in graph_model.modules():
                if isinstance(module, (QuantizedConvBatchNorm2d, QuantConvTransposeBatchNorm2d, QuantConv2d, QuantConvTranspose2d, QuantLinear)):
            ...
            assert count_QuantizeModule == 2, "In model {}, the QuantizedModule num should ne 2".format(
                each_fp_model.__class__.__name__)
        ...
        for module in graph_model.modules():
            if isinstance(module, (QuantizedConvBatchNorm2d, QuantConvTransposeBatchNorm2d, QuantConv2d, QuantConvTranspose2d, QuantLinear)):
                ...
                count_QuantizeModule += 1
        out_opt_fx_graph = graph_model(*example_inputs)
        assert count_QuantizeModule == 1, "In model {}, the QuantizedModule num should ne 1".format(
            each_fp_model.__class__.__name__)
        assert torch.allclose(out_fp32[0], out_opt_fx_graph[0], atol=ABSOLUTE_TOL)
        assert torch.allclose(out_fp32[1], out_opt_fx_graph[1], atol=ABSOLUTE_TOL)
...
def test_torch_module_used_over_once_optim_strategy(tmpdir: str):
    ...
    '''
    INT8_PER_TENSOR_MSE_POW2_SPEC = QuantizationSpec(dtype=Dtype.int8,
                                                     qscheme=QSchemeType.per_tensor,
                                                     ...
                                                     round_method=RoundType.half_even,
                                                     is_dynamic=False)
    powof2_mse_tensor_config = QuantizationConfig(input_tensors=INT8_PER_TENSOR_MSE_POW2_SPEC,
...
def test_torch_sg_bn2d_to_conv2d_optim_strategy():
    ...
    for graph_model in [graph_model_1, graph_model_2]:
        opt_graph_module = trans_opsfunc_2_quant_module(graph_model)  # default op
    ...
    for graph_model in [graph_model_1, graph_model_2]:
        ...
        quantized_model = quantizer.quantize_model(graph_model,
                                                   [torch.rand(4, 3, 32, 32).to(torch_device) for _ in range(3)])

        assert fx_contain_module_num(quantized_model, ScaledFakeQuantize) == 7, "The total quantizer in this model should be 7"
...
class TinyMean2GAP(nn.Module):
    ...
    def forward(self, x0, x1, x2, x3, x4, x5, x6, x7, x8):
        ...
        x2 = torch.mean(x2, dim=(2, 3),
                        keepdim=True)  # onnx: GlobalAveragePool  # torch2.5 & cap: torch.ops.aten.mean.dim()
        # input output  -> 2
        x3 = self.pool1(
            x3)  # onnx: GlobalAveragePool  #torch2.5 & torchcap torch.ops.aten.adaptive_avg_pool2d.default()
        # input output  -> 2
...
def test_mean_2_pooling_strategy(tmpdir: str):
    ...
    example_inputs = (torch.rand(1, 3, 6, 6).to(torch_device), torch.rand(1, 3, 8, 8).to(torch_device),
                      torch.rand(1, 3, 10, 10).to(torch_device), torch.rand(1, 3, 12, 12).to(torch_device),
                      torch.rand(1, 3, 14, 14).to(torch_device), torch.rand(1, 3, 16, 16).to(torch_device),
                      torch.rand(1, 3, 18, 18).to(torch_device), torch.rand(1, 3, 20, 20).to(torch_device),
    ...
    assert all([torch.allclose(x[0], x[1]) for x in zip(fp_out, graph_out)])
    # ========== test quant pipeline===============
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    for each_quant_config in [emp_quant_config, quant_config]:
        ...
        out_2 = quantized_model.eval()(*example_inputs)
        if each_quant_config == emp_quant_config:
            assert [i for i, x in enumerate(zip(fp_out, out_2)) if torch.allclose(x[0], x[1])] == [0, 1, 5, 7]
        assert fx_contains_op_num(graph_model, is_mean_node) == 2
...
def test_torch_convert_split_2_slice_strategy(tmpdir: str):
    ...
    for graph_model in [graph_model_1, graph_model_2]:
        ...
        assert all([torch.allclose(x[0], x[1]) for x in zip(out, graph_out)])
    # ========== test quant pipeline===============
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    for each_quant_config in [emp_quant_config, quant_config]:
        ...
        if each_quant_config == emp_quant_config:
            assert all([torch.allclose(x[0], x[1]) for x in zip(out, out_2)])
        assert fx_contains_op_num(graph_model, is_split_node) == 0
        assert fx_contains_op_num(graph_model, is_slice_node) == 16
...
def test_torch_adaptiveavgpool2d_2_qtadaptiveavgpool2d_strategy(tmpdir: str):
    ...
    assert all([torch.allclose(x[0], x[1]) for x in zip(fp_out, gp_out)]) is True
        ...
        for quantizer in [unify_quantizer, fx_quantizer]:
            ...
            for each_format_model in input_models:
                input_args = {
                    "model": each_format_model,
                    "args": example_inputs,
                    'calibdata': [quant_inputs]
                } if isinstance(quantizer, FxGraphQuantizer) else {
                    "model": each_format_model,
                    'dataloader': [quant_inputs]
                }
                quantized_model = quantizer.quantize_model(**input_args)
...
def test_torch_postquant_concat_strategy(tmpdir: str):
    ...
    gp_out = graph_model(*example_inputs)
    assert torch.allclose(fp_out, gp_out)
    opt_graph = opt_after_qt.ApplyConstrain2ConcatQOPass()(graph_model)  # skip
    # ========== test quant pipeline===============
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    for each_quant_config in [quant_config, emp_quant_config]:
        ...
        for quantizer in [fx_quantizer, unify_quantizer]:
            ...
            for each_format_model in input_models:
                ...
                if fx_contain_module_num(quantized_model, ScaledFakeQuantize):
                    ...
                    opt_graph = opt_after_qt.ApplyConstrain2ConcatQOPass()(quantized_model)
                    ...
                    opt_graph = opt_after_qt.ApplyConstrain2ConcatQOPass()(opt_graph)
    torch.cuda.empty_cache()
...
def test_torch_align_single_in_out_strategy(tmpdir: str):
    ...
    gp_out = graph_model(*example_inputs)
    assert torch.allclose(fp_out, gp_out)
    opt_graph = opt_after_qt.AlignSingleInOutOpScaleQOPass([torch.ops.aten.sigmoid.default])(graph_model)  # skip
    # ========== test quant pipeline===============
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    opt_module = opt_after_qt.AlignSingleInOutOpScaleQOPass([torch.ops.aten.hardsigmoid.default])
    for each_quant_config in [quant_config, emp_quant_config]:
        unify_quantizer = ModelQuantizer(each_quant_config)
        fx_quantizer = FxGraphQuantizer(each_quant_config)
        for quantizer in [fx_quantizer, unify_quantizer]:
...
def test_torch_align_single_in_out_module_strategy(tmpdir: str):
    ...
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    opt_module = opt_after_qt.AlignSingleInOutModuleScaleQOPass((QuantAvgPool2d, QuantAdaptiveAvgPool2d))
    for each_quant_config in [quant_config, emp_quant_config]:
        unify_quantizer = ModelQuantizer(each_quant_config)
        fx_quantizer = FxGraphQuantizer(each_quant_config)
        for quantizer in [fx_quantizer, unify_quantizer]:
...
def test_torch_adjust_shift_read_strategy(tmpdir: str):
    ...
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    opt_module = opt_after_qt.AdjustShiftReadQOPass()
    for each_quant_config in [quant_config, emp_quant_config]:
        unify_quantizer = ModelQuantizer(each_quant_config)
        fx_quantizer = FxGraphQuantizer(each_quant_config)
        for quantizer in [fx_quantizer, unify_quantizer]:
...
def test_torch_adjust_shift_write_strategy(tmpdir: str):
    ...
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    opt_module = opt_after_qt.AdjustShiftWriteQOPass()
    for each_quant_config in [quant_config, emp_quant_config]:
        unify_quantizer = ModelQuantizer(each_quant_config)
        fx_quantizer = FxGraphQuantizer(each_quant_config)
        for quantizer in [fx_quantizer, unify_quantizer]:
...
def test_torch_adjust_shift_cut_strategy(tmpdir: str):
    ...
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    opt_module = opt_after_qt.AdjustShiftCutQOPass()
    for each_quant_config in [quant_config, emp_quant_config]:
        unify_quantizer = ModelQuantizer(each_quant_config)
        fx_quantizer = FxGraphQuantizer(each_quant_config)
        for quantizer in [fx_quantizer, unify_quantizer]:
...
def test_torch_adjust_shift_bias_strategy(tmpdir: str):
    ...
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    opt_module = opt_after_qt.AdjustShiftBiasQOPass()
    for each_quant_config in [quant_config, emp_quant_config]:
        unify_quantizer = ModelQuantizer(each_quant_config)
        fx_quantizer = FxGraphQuantizer(each_quant_config)
        for quantizer in [fx_quantizer, unify_quantizer]:
...
def test_torch_adjust_hard_sigmoid_strategy(tmpdir: str):
    ...
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    opt_module = opt_after_qt.AdjustHardSigmoidQOPass()
    for each_quant_config in [quant_config, emp_quant_config]:
        unify_quantizer = ModelQuantizer(each_quant_config)
        fx_quantizer = FxGraphQuantizer(each_quant_config)
        for quantizer in [fx_quantizer, unify_quantizer]:
...
def test_torch_adjust_shift_swishd_strategy(tmpdir: str):
    ...
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    opt_module = opt_after_qt.AdjustShiftSwishQOPass()
    for each_quant_config in [quant_config, emp_quant_config]:
        unify_quantizer = ModelQuantizer(each_quant_config)
        fx_quantizer = FxGraphQuantizer(each_quant_config)
        for quantizer in [fx_quantizer, unify_quantizer]:
...
def test_torch_convert_hard_sigmoid_dpu_strategy(tmpdir: str):
    ...
    gp_out = graph_model(*example_inputs)
    assert torch.allclose(fp_out, gp_out)
    opt_graph = opt_after_qt.ConvertHardSigmoidDpuVersionQOPass()(graph_model)  # skip
    # ========== test quant pipeline===============
    emp_config = QuantizationConfig()
    emp_quant_config = Config(global_quant_config=emp_config, quant_mode=QuantizationMode.fx_graph_mode)
    opt_module = opt_after_qt.ConvertHardSigmoidDpuVersionQOPass()
    for each_quant_config in [quant_config, emp_quant_config]:
        unify_quantizer = ModelQuantizer(each_quant_config)
        fx_quantizer = FxGraphQuantizer(each_quant_config)
        for quantizer in [fx_quantizer, unify_quantizer]:
...
INT8_PER_TENSOR_SPEC_W_A = QuantizationSpec(dtype=Dtype.int8,
                                            qscheme=QSchemeType.per_tensor,
                                            observer_cls=PerTensorPowOf2MinMaxObserver,
                                            symmetric=True,
                                            scale_type=ScaleType.float,
                                            round_method=RoundType.half_even,
                                            is_dynamic=False)
INT8_PER_TENSOR_SPEC_BIAS = QuantizationSpec(dtype=Dtype.int32,
                                             qscheme=QSchemeType.per_tensor,
                                             observer_cls=PerTensorPowOf2MinMaxObserver,
                                             symmetric=True,
                                             scale_type=ScaleType.float,
                                             round_method=RoundType.half_even,
                                             is_dynamic=False)
quant_tensor_W_A_8_B_32_config = QuantizationConfig(input_tensors=INT8_PER_TENSOR_SPEC_W_A,
                                                    output_tensors=INT8_PER_TENSOR_SPEC_W_A,
                                                    weight=INT8_PER_TENSOR_SPEC_W_A,
                                                    bias=INT8_PER_TENSOR_SPEC_BIAS)
quant_W_A_8_B_32_config = Config(global_quant_config=quant_tensor_W_A_8_B_32_config,
        ...
        for quantizer in [unify_quantizer, fx_quantizer]:
            ...
            for each_format_model in input_models:
                ...
                if fx_contain_module_num(quantized_model, ScaledFakeQuantize):
                    ...
                    assert act_quantizer.scale.numel() == 1 and conv_w_quantizer.scale.numel(
                    ) == 1 and conv_b_quantizer.scale.numel() == 1
                    ...
                    assert act_quantizer.scale.detach().clone() * conv_w_quantizer.scale.detach().clone(
