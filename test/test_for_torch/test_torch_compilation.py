...
from quark.torch.quantization.observer.observer import (PerTensorMinMaxObserver, PerChannelMinMaxObserver,
                                                        PerGroupMinMaxObserver, PerBlockMXObserver)
...
def quantize_and_compile_model(device, QUANT_SPEC):
    ...
    freezed_quant_model = quantizer.freeze(quant_model)
    ...
    freezed_quant_model = torch.compile(backend=custom_backend)(freezed_quant_model)
    output2 = freezed_quant_model(input_data)
...
@pytest.mark.parametrize("dtype,element_dtype", [
    (Dtype.mx6, None),
    (Dtype.mx9, None),
    (Dtype.mx, Dtype.int8),
