...
Tests that use `torch_device` but can only be run on GPU can be skipped on CPU with the `@require_torch_gpu` decorator as shown at [example](/test/test_for_torch/test_lsq_FakeQuantize.py#L34).
