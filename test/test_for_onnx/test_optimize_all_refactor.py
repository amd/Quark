...
def prepare_config_convert_bn_to_conv():
...
def prepare_config_convert_clip_to_relu():
    quant_config = QConfig(
        global_config=QLayerConfig(activation=XInt8Spec(), weight=XInt8Spec()), ConvertClipToRelu=True
    )
    return quant_config
...
def tensor_quantize(input_data, input_shape, model_type, output_dir: str):
    ...
    if model_type == "convert_bn_to_conv":
        quant_config = prepare_config_convert_bn_to_conv()
    elif model_type == "convert_clip_to_relu":
        quant_config = prepare_config_convert_clip_to_relu()
