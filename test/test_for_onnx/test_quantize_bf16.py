...
def prepare_add_model(tmp_path):
    ...
    node = helper.make_node(
        'Add',
        inputs=['input', 'initializer_tensor'],
        outputs=['output']
    )
...
def prepare_cast_model(tmp_path):
    ...
    cast_node = helper.make_node(op_type = "Cast", inputs = ["input"], outputs = ["cast_output"], to = TensorProto.FLOAT, name = "CastToFloat32")

    weight_tensor = helper.make_tensor(name = "conv_weight", data_type = TensorProto.FLOAT, dims = [1, 3, 3, 3], vals = np.random.randn(1, 3, 3, 3).astype(np.float32).flatten().tolist())

    conv_node = helper.make_node(op_type = "Conv", inputs = ["cast_output", "conv_weight"], outputs = ["output"], name = "ConvNode")
...
def prepare_2output_model(tmp_path):
    ...
    conv1_weight = helper.make_tensor(name = "conv1_weight", data_type = TensorProto.FLOAT, dims = [4, 3, 3, 3], vals = np.random.randn(4, 3, 3, 3).astype(np.float32).flatten().tolist())

    conv2_weight = helper.make_tensor(name = "conv2_weight", data_type = TensorProto.FLOAT, dims = [2, 4, 3, 3], vals = np.random.randn(2, 4, 3, 3).astype(np.float32).flatten().tolist())

    conv1_node = helper.make_node(op_type = "Conv", inputs = ["input", "conv1_weight"], outputs = ["conv1_output"], name = "Conv1")

    conv2_node = helper.make_node(op_type = "Conv", inputs = ["conv1_output", "conv2_weight"], outputs = ["output"], name = "Conv2")
