...
output_tensor = np.array(
    [
        [
            [
                [0.13671875, 0.04296875, -0.09375, -0.1640625],
                [0.03125, -0.1328125, 0.01953125, -0.078125],
                [0.0078125, 0.15625, 0.3046875, -0.00390625],
                [0.03125, 0.01953125, 0.1171875, 0.16796875],
            ]
        ]
    ]
).astype(np.float32)
...
def prepare_config():
    bias_correction_algo = BiasCorrectionConfig()
    quant_config = QConfig(
        global_config=QLayerConfig(activation=XInt8Spec(), weight=XInt8Spec()), algo_config=[bias_correction_algo]
    )
    return quant_config
...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_quantize_bias_correction(self, tmpdir: str):
