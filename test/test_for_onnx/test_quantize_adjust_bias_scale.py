...
def prepare_model(output_dir):
    ...
    mul_weight_tensor = helper.make_tensor(
        name='mul_weight',
        data_type=TensorProto.FLOAT,
        dims=mul_weight.shape,
        vals=mul_weight.flatten().tolist()
    )
    mul_node = helper.make_node(
        'Mul',
        inputs=['mul_input', 'mul_weight'],
        name='mul_node',
        outputs=['mul_output']
    )
    ...
    reshape_shape_tensor = helper.make_tensor(
        ...
        dims=reshape_shape.shape,
        vals=reshape_shape.tolist()
    )
    reshape_node = helper.make_node(
        'Reshape',
        inputs=['concat_output', 'reshape_shape'],
    ...
    gemm_weight_tensor = helper.make_tensor(
        name='gemm_weight',
        data_type=TensorProto.FLOAT,
        dims=gemm_weight.shape,
        vals=gemm_weight.flatten().tolist()
    )
    gemm_node = helper.make_node(
        'Gemm',
        inputs=['reshaped_output', 'gemm_weight'],
        outputs=['output'],
        name='gemm_node'
    )
