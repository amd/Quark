...
output_tensor_static = np.array([[[[0.25390625, 0.16015625, 0.0234375, -0.046875],
...
output_tensor_dynamic = np.array([[[[0.25061864, 0.15933731, 0.0213424, -0.04940237],
                                    [0.1462864, -0.02025713, 0.13545156, 0.03660944],
                                    [0.12172377, 0.26719928, 0.41983452, 0.11198696],
                                    [0.14806628, 0.13565212, 0.23285973, 0.28434148]]]]).astype(np.float32)
...
def prepare_static_config():
    ...
    quant_config.crypto_mode = True
    return Config(global_quant_config=quant_config)


def prepare_dynamic_config():
    quant_config = get_default_config('UINT8_DYNAMIC_QUANT')
    quant_config.crypto_mode = True
    return Config(global_quant_config=quant_config)
...
def quantize_dynamic(quantizer, input_model_path, output_model_path):
    input_model = onnx.load(input_model_path)
    output_model = quantizer.quantize_model(input_model)
    onnx.save(output_model, output_model_path)
    print('Dynamic quantized the ONNX model and saved it at:', output_model_path)
    return output_model_path
...
class TestTensorQuantize(unittest.TestCase):
    ...
    @use_temporary_directory
    def test_dynamic_quantize_crypto_mode(self, tmpdir: str):
        output = tensor_dynamic_quantize(tmpdir)
        comp_equal = np.allclose(output, output_tensor_dynamic, atol=1e-1)
        self.assertEqual(np.all(comp_equal), True)
