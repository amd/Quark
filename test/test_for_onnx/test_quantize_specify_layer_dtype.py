...
from quark.onnx import Int16Spec, ModelQuantizer, QConfig, QLayerConfig, XInt8Spec
...
def prepare_config(specify_layer_types):
    quant_config = QConfig(
        global_config=QLayerConfig(activation=XInt8Spec(), weight=XInt8Spec()),
        specific_layer_config=specify_layer_types,
        SimplifyModel=False,
        Int32Bias=False,
    )
    return quant_config
...
def tensor_quantize(output_dir, specify_layer_types):
    input_model_path, output_model_path = prepare_model(output_dir)
    data_reader = prepare_data()
    quant_config = prepare_config(specify_layer_types)
...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_subgraph(self, tmpdir: str):
        output = tensor_quantize(
            tmpdir, {QLayerConfig(input_tensors=Int16Spec(), weight=Int16Spec()): [(["/conv1/Conv"], ["/conv2/Conv"])]}
        )
        comp_equal = np.allclose(output, output_golden, atol=1e-1)
        self.assertEqual(comp_equal, True)

    @use_temporary_directory
    def test_single_nodes(self, tmpdir: str):
        output = tensor_quantize(
            tmpdir, {QLayerConfig(input_tensors=Int16Spec(), weight=Int16Spec()): ["/conv1/Conv", "/conv2/Conv"]}
        )
        comp_equal = np.allclose(output, output_golden, atol=1e-1)
    ...
    @use_temporary_directory
    def test_pattern(self, tmpdir: str):
        output = tensor_quantize(
            tmpdir, {QLayerConfig(input_tensors=Int16Spec(), weight=Int16Spec()): ["^/conv1/.*", "^/conv2/.*"]}
        )
        comp_equal = np.allclose(output, output_golden, atol=1e-1)
        self.assertEqual(comp_equal, True)
