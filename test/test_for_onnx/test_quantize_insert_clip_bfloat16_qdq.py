...
golden_output = np.array([[-0.3125, -0.09667969, -0.359375, -0.02172852, 0.01782227,
                           -0.828125, 0.8046875, -0.25195312, -0.06494141, -0.66015625]]).astype(np.float32)
...
def prepare_config(config, is_insert_clip_bfloat16_qdq):
    config_copy = copy.deepcopy(config)
    quant_config = Config(global_quant_config=config_copy)
    if is_insert_clip_bfloat16_qdq:
        config_copy.extra_options['BF16WithClip'] = True
    else:
        config_copy.extra_options['BF16WithClip'] = False
    return quant_config
...
def tensor_quantize(input_data, is_replace_bfloat16_qdq_clip, output_dir):
    ...
    quant_config = prepare_config(BF16_CONFIG, is_replace_bfloat16_qdq_clip)
