...
input_tensor_x = np.array([[[[0.26921557, 0.79500909, 0.6102178, 0.04375664],
...
input_tensor_y = np.array([[0.26921557, 0.79500909, 0.6102178, 0.04375664],
                           [0.06221361, 0.98258356, 0.38635129, 0.06492238],
                           [0.49631707, 0.35442799, 0.51719146, 0.52100111],
                           [0.04145599, 0.88960236, 0.50627326, 0.57204613]]).astype(np.float32)
...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_quantize_weights_only_fastfinetune(self, tmpdir: str):
        ...
        config = XINT8_WEIGHTSONLY_ADAROUND_CONFIG
        output = tensor_quantize(config, tmpdir)
        comp_equal = np.allclose(output, output_tensor, atol=1e-1)
        print("output", output)
        print("output_tensor", output_tensor)
