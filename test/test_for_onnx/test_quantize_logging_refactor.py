...
def quantize_static(quantizer, input_model_path, output_model_path, data_reader):
    # check debug
    quantizer.config.extra_options["LogSeverityLevel"] = 0
    quantizer.set_logging_level()
    logger.debug("Successfully checked Log Debug.")

    # checkwarning
    quantizer.config.extra_options["LogSeverityLevel"] = 2
    quantizer.set_logging_level()
    logger.warning("Successfully checked Log Warning.")

    # check error
    quantizer.config.extra_options["LogSeverityLevel"] = 3
    quantizer.set_logging_level()
    try:
    ...
    # check critical
    quantizer.config.extra_options["LogSeverityLevel"] = 4
    quantizer.set_logging_level()
    try:
    ...
    # check info
    quantizer.config.extra_options["LogSeverityLevel"] = 1
    quantizer.set_logging_level()
