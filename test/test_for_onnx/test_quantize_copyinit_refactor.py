...
def tensor_quantize(output_dir):
    ...
    quant_config = QConfig(QLayerConfig(activation=UInt8Spec(), weight=Int8Spec()))
    ...
    new_quant_config = QConfig(
        QLayerConfig(activation=UInt8Spec(), weight=Int8Spec()), CopySharedInit=["Conv", "ConvTranspose", "Gemm"]
    )
