...
output_golden = np.array([[-0.46397364]], dtype=np.float32)
...
def prepare_subgraphs_to_exclude_config():
    """
    config_copy = copy.deepcopy(U8S8_AAWS_CONFIG)
    config_copy.subgraphs_to_exclude = [(["/conv1/Conv"], ["/fc/Gemm"])]
    quant_config = Config(global_quant_config=config_copy)
    """
    quant_config = QConfig(
        global_config=QLayerConfig(activation=UInt8Spec(), weight=Int8Spec()), exclude=[(["/conv1/Conv"], ["/fc/Gemm"])]
    )
    return quant_config


def prepare_nodes_to_exclude_config():
    """
    config_copy = copy.deepcopy(U8S8_AAWS_CONFIG)
    config_copy.nodes_to_exclude = ["^/fc/.*"]
    quant_config = Config(global_quant_config=config_copy)
    """
    quant_config = QConfig(global_config=QLayerConfig(activation=UInt8Spec(), weight=Int8Spec()), exclude=["^/fc/.*"])
    return quant_config
...
def tensor_quantize_exclude_subgraphs(output_dir):
    input_model_path, output_model_path = prepare_model(output_dir)
    data_reader = prepare_data()
    quant_config = prepare_subgraphs_to_exclude_config()
...
def tensor_quantize_exclude_nodes(output_dir):
    input_model_path, output_model_path = prepare_model(output_dir)
    data_reader = prepare_data()
    quant_config = prepare_nodes_to_exclude_config()
    quantizer = prepare_quantizer(quant_config)
    quantized_model_path = quantize_static(quantizer, input_model_path, output_model_path, data_reader)
    output = infer_quantized_model(quantized_model_path)
    return output
...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_quantize_subgraphs_to_exclude(self, tmpdir: str):
        output = tensor_quantize_exclude_subgraphs(tmpdir)
        comp_equal = np.allclose(output, output_golden, atol=1e-1)
        self.assertEqual(comp_equal, True)

    @use_temporary_directory
    def test_quantize_nodes_to_exclude(self, tmpdir: str):
        output = tensor_quantize_exclude_nodes(tmpdir)
        comp_equal = np.allclose(output, output_golden, atol=1e-1)
        self.assertEqual(comp_equal, True)
