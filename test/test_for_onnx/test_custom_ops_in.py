...
if __name__ == "__main__":
    ...
    def _create_model(data_type: TensorProto.DataType, data_shape: Tuple[int, ...], output_dir: str) -> None:
        ...
        in_param_nodes = [
            helper.make_node("Constant", [], ["gamma"],
                             value=onnx.helper.make_tensor("y_scale", data_type, data_shape, gamma)),
            helper.make_node("Constant", [], ["beta"],
                             value=onnx.helper.make_tensor("y_zero_point", data_type, data_shape, beta)),
        ]
