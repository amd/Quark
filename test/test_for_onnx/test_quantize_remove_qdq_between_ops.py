...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_quantize_MultiMulAddModel(self, tmpdir: str):
        with self.assertLogs('quark.onnx.tools.remove_qdq_between_ops_screen', level='INFO') as cm:
            output, quantized_model_path = tensor_quantize(input_data, [('Conv', 'Relu'), ('Conv', 'LeakyRelu'), ('Conv', 'PRelu'), ('Mul', 'Add')], tmpdir)
        self.assertTrue(any("Removed QuantizeLinear & DequantizeLinear operations: " in message for message in cm.output))
