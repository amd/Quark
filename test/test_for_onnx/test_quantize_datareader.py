...
import numpy as np
import onnxruntime
from quark.onnx.quant_utils import RandomDataReader
...
from quark.shares.utils.testing_utils import use_temporary_directory, delete_directory_content
...
calib_data = np.array([[[[0.9296161, 0.31637555, 0.18391882, 0.20456028],
...
golden_output = np.array([[[[-0.52279645, -0.39127532, -0.3189387, -0.25646618],
                            [-0.04274436, -0.28934646, -0.26633024, -0.09535281],
                            [-0.20714575, -0.13809717, -0.11179294, -0.28934646],
                            [-0.12165703, -0.23016195, -0.19399364, -0.07233661]]]]).astype(np.float32)
...
def quantize_static(quantizer, input_model_path, output_model_path, calibration_data_reader=None, calibration_data_path=None):
    quantizer.quantize_model(input_model_path, output_model_path, calibration_data_reader=calibration_data_reader, calibration_data_path=calibration_data_path)
...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_random_data_reader_with_shape(self, tmpdir: str):
        ...
        quantized_model_path = quantize_static(quantizer, input_model_path, output_model_path, calibration_data_reader=data_reader)
    ...
    def test_quantize_CalibrationDataPath(self, tmpdir: str):
        ...
        quant_config = prepare_config()
        quantizer = prepare_quantizer(quant_config)
        quantized_model_path = quantize_static(quantizer, input_model_path, output_model_path, calibration_data_reader=None, calibration_data_path=calib_data_dir)
        output = infer_quantized_model(quantized_model_path)
        comp_equal = np.allclose(output, golden_output, atol=1e-1)
        self.assertEqual(np.all(comp_equal), True)
    ...
    def test_data_reader_none(self, tmpdir: str):
        ...
        with self.assertRaises(Exception) as context:
            quantized_model_path = quantize_static(quantizer, input_model_path, output_model_path, calibration_data_reader=None)

        self.assertIn("calibration data reader is required for static quantization", str(context.exception))
