...
adaround_output_tensor = np.array(
    [
        [
            [
...
adaquant_output_tensor = np.array(
    [
        [
            [
                [-0.16577704, 0.24866557, -0.15886967, 1.3538458],
                [-0.08288852, 0.31083196, 0.58712703, 0.34536883],
                [0.35227624, -0.15886967, -0.13124016, 1.2018836],
                [-0.1519623, -0.02762951, -0.14505492, 1.4989008],
                [-0.16577704, 0.1519623, 1.409105, -0.1519623],
                [-0.08288852, 0.09670328, 0.6562008, 0.52496064],
                [1.0015696, -0.01381475, 0.29701722, -0.0897959],
                [-0.13814753, 0.22103606, 1.3054942, -0.15886967],
                [0.06907377, -0.13124016, -0.13124016, 1.3952901],
                [0.5111459, -0.16577704, -0.13124016, 1.1051803],
                [-0.13124016, 0.06216639, -0.13124016, 1.3952901],
                [1.409105, -0.16577704, 0.13124016, -0.14505492],
                [1.0568286, -0.13814753, 0.43516475, -0.10361066],
                [0.37990573, 0.00690738, 0.897959, -0.08288852],
                [1.0430139, -0.08288852, 0.36609098, -0.09670328],
                [0.6009418, -0.06907377, 0.79434836, -0.0897959],
            ]
        ],
    ]
).astype(np.float32)
...
def prepare_adaround_config(MemOptLevel: int = 0, SelectMaxMemLayer: bool = False):
...
def prepare_adaquant_config(MemOptLevel: int = 0, SelectMaxMemLayer: bool = False):
...
class TestMemoryProfiling(unittest.TestCase):
    ...
    def test_quantize_fastft_adaround(self, tmpdir: str):
        quant_config = prepare_adaround_config(MemOptLevel=0, SelectMaxMemLayer=True)
        output = tensor_quantize(tmpdir, quant_config)
        comp_equal = np.allclose(output, adaround_output_tensor, atol=1e-1)
        self.assertEqual(np.all(comp_equal), True)

    @use_temporary_directory
    def test_quantize_fastft_adaquant(self, tmpdir: str):
        quant_config = prepare_adaquant_config(MemOptLevel=0, SelectMaxMemLayer=True)
        output = tensor_quantize(tmpdir, quant_config)
        comp_equal = np.allclose(output, adaquant_output_tensor, atol=1e-1)
        self.assertEqual(np.all(comp_equal), True)

    # @use_temporary_directory
    # def test_gpu_memory_profiling(self, tmpdir: str):
    #     save_path = os.path.join(tmpdir, "gpu_memory_usage.png")
    #     monitor_rocm_memory(gpu_ids=[6], interval=1, duration=10, save_path=save_path, save_csv=False)
