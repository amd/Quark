...
from quark.onnx import Int8Spec, ModelQuantizer, QConfig, QLayerConfig, get_library_path
...
output_golden = np.array([[-0.03653336]], dtype=np.float32)
...
def prepare_config():
    quant_config = QConfig(global_config=QLayerConfig(activation=Int8Spec(), weight=Int8Spec()), Int16Bias=True)
    return quant_config
...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_quantize_int16_bias(self, tmpdir: str):
