...
def prepare_config():
    quant_config = Config(global_quant_config=UINT8_DYNAMIC_QUANT_CONFIG)
    return quant_config
...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_quantize_dynamic(self, tmpdir: str):
        output = tensor_quantize(tmpdir)

        golden = np.array([[-0.50937814]], dtype=np.float32)
