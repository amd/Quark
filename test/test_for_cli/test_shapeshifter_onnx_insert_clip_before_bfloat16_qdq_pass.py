...
def quantize_onnx_model(output_dir, input_data):
    ...
    Returns:
        tuple: A tuple containing:
            ...
            - insert_clip_before_bfloat16_qdq_model_path: Path for the insert_clip_before_bfloat16_qdq_model quantized ONNX model.
    """
    quantized_model_path = Path(output_dir, "onnx_quantized_qdq.onnx").as_posix()
    insert_clip_before_bfloat16_qdq_model_path = Path(
        output_dir, "insert_clip_before_bfloat16_qdq_model.onnx"
    ).as_posix()
    data_reader = DataReader(input_data)
    input_model_path = prepare_float_onnx_model(output_dir)
    quant_config = QConfig(
        global_config=QLayerConfig(activation=BFloat16Spec(), weight=BFloat16Spec()), BF16WithClip=False
    )
    quantizer = ModelQuantizer(quant_config)
    quantizer.quantize_model(input_model_path, quantized_model_path, data_reader)
    return quantized_model_path, insert_clip_before_bfloat16_qdq_model_path
...
def prepare_yaml(output_dir, onnx_model_path, onnx_optimized_model_path):
    ...
    """
    yaml_path = Path(output_dir, "insert_clip_before_bfloat16_qdq.yaml").as_posix()
    config = {
        "input_model_path": onnx_model_path,
        "passes": {
            "onnx_insert_clip_before_bfloat16_qdq": {
                "insert_clip_before_bfloat16_qdq": True,
            }
        },
...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_onnx_adapter_onnx_insert_clip_before_bfloat16_qdq(self, tmpdir: str):
        quantized_model_path, insert_clip_before_bfloat16_qdq_model_path = quantize_onnx_model(tmpdir, input_data)
        yaml_path = prepare_yaml(tmpdir, quantized_model_path, insert_clip_before_bfloat16_qdq_model_path)
        cli(["shapeshifter", yaml_path])
        flag = check_model(insert_clip_before_bfloat16_qdq_model_path)
