...
def prepare_yaml(output_dir, onnx_model_path, onnx_optimized_model_path):
    yaml_path = Path(output_dir, "fix_shapes.yaml").as_posix()
    config = {
        "input_model_path": onnx_model_path,
        "passes": {
            "onnx_fix_shapes": {
                "input_output_name_shapes": {"input": [1, 3, 4, 4], "output": [1, 1]},
            }
        },
...
def check_static_shape(onnx_optimized_model_path):
    model = onnx.load(onnx_optimized_model_path)

    graph = model.graph
    all_value_infos = list(graph.input) + list(graph.value_info) + list(graph.output)
    ...
    for value_info in all_value_infos:
        ...
        for dim in t.shape.dim:
            # symbolic dimension → dynamic
            if dim.dim_param:
                return False
            # 0-size dimension → dynamic-like
            if dim.dim_value == 0:
                return False
...
class TestTensorQuantize(unittest.TestCase):
    ...
    def test_onnx_adapter_onnx_fix_shapes_pass(self, tmpdir: str):
        ...
        flag = check_static_shape(onnx_optimized_model_path)
