...
description: >
  Diagnose failed Quark installation, PTQ execution, script generation, or export attempts. Use when the user reports
  an error, stack trace, invalid artifact, missing dependency, CUDA OOM, version mismatch, or unexpected PTQ results.
  Trigger for "Quark error", "PTQ failed", "quantization crashed", "CUDA out of memory", "import error",
  "model loading failed", "wrong results", any Python traceback mentioning quark/torch/transformers, or when
  the user pastes an error message related to Quark workflows.
layer: l1-atomic
