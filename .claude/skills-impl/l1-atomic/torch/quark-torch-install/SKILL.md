...
description: >
  Install or verify the correct PyTorch build for a user's accelerator backend before Quark installation. Use when the
  user needs PyTorch set up, reports torch version conflicts, CUDA/ROCm package mismatches, or when torch.cuda.is_available()
  returns False. Trigger for "install PyTorch", "pip install torch", "set up torch for ROCm", "set up torch for CUDA",
  "torch version mismatch", "CPU-only torch installed", or any request to get the correct PyTorch build running.
  Also trigger when quark-install reports that PyTorch is missing or mismatched before proceeding.
layer: l1-atomic
primary_artifact: pytorch_install_result.json
source_knowledge:
...
Schema: [`pytorch_install_result.schema.json`](../../../shared/contracts/pytorch_install_result.schema.json)
