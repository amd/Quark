...
from utils import box_iou, clip_coords, letterbox, non_max_suppression, plot_one_box, scale_coords, xywh2xyxy
...
class LoadImagesAndLabels(Dataset):
    def __init__(
        ...
        if self.rect:
            ...
            self.batch_shapes = np.ceil(np.array(shapes) * img_size / 32.0 + pad).astype(int) * 32
        ...
        for i, file in enumerate(pbar):
            ...
            pbar.desc = (
                "Caching labels %s (%g found, %g missing, %g empty,\
    ...
    def collate_fn(batch):
        img, label, path, shapes = zip(*batch, strict=False)
...
def post_process(x):
    ...
    for i in range(3):
        ...
        out = out.view(bs, 3, 85, temp[i], temp[i]).permute(0, 1, 3, 4, 2).contiguous()
...
def test(
        ...
        for si, pred in enumerate(output):
            ...
            if nl:
                ...
                for cls in torch.unique(tcls_tensor):
                    ...
                    if pi.shape[0]:
                        ious, i = box_iou(pred[pi, :4], tbox[ti].cpu()).max(1)
    ...
    stats = [np.concatenate(x, 0) for x in zip(*stats, strict=False)]
    if len(stats):
        p, r, ap, f1, ap_class = ap_per_class(*stats)
        if niou > 1:
            p, r, ap, f1 = p[:, 0], r[:, 0], ap.mean(1), ap[:, 0]
    ...
    if verbose or save_json:
        ...
        print(
            "Speed: %.1f/%.1f/%.1f ms \
            inference/NMS/total per %gx%g image at batch-size %g"
            % t
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", type=str, default="coco2017.data", help="Path of *.data")
    ...
    parser.add_argument("--onnx_runtime", action="store_true", help="Use onnx runtime")
    parser.add_argument("--onnx_weights", default="yolov3-8.onnx", nargs="+", type=str, help="Path of onnx weights")
    parser.add_argument("--single-cls", action="store_true", help="Run as single-class dataset")
