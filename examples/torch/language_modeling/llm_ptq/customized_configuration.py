...
DEPRECATED_QUANT_SCHEME = {
    ...
    "w_mx_fp4_a_mx_fp6_sym": "w_mxfp4_a_mxfp6_e2m3",
...
SUPPORTED_QUANT_SCHEME = (
    [
        "no_quant",  # This scheme is useful to test some pre-processing methods, that should not impact the model quality before quantization (rotation, smoothquant, etc.)
        "w_bfp16",
        "w_bfp16_a_bfp16",
        ...
        "w_fp8_per_channel_static_a_fp8_per_token_dynamic",
        "w_int2_per_group_asym",
        "w_int3_per_group_asym",
        "w_int3_per_channel_asym",
...
W_BFP16_SPEC = BFP16Spec(ch_axis=-1).to_quantization_spec()

A_BFP16_SPEC = BFP16Spec(ch_axis=-1).to_quantization_spec()

W_MX6_SPEC = MX6Spec(ch_axis=-1, block_size=32).to_quantization_spec()

A_MX6_SPEC = MX6Spec(ch_axis=-1, block_size=32).to_quantization_spec()
...
# BFloat16 config
BFP16_PER_GROUP_SYM_SPEC = BFP16Spec(ch_axis=-1).to_quantization_spec()
...
INT3_PER_CHANNEL_ASYM_SPEC = Int3PerChannelSpec(
    symmetric=False, scale_type="float", round_method="half_even", ch_axis=0, is_dynamic=False
).to_quantization_spec()
INT3_PER_GROUP_ASYM_SPEC = Int3PerGroupSpec(
    symmetric=False, scale_type="float", round_method="half_even", ch_axis=1, is_dynamic=False, group_size=128
).to_quantization_spec()
...
W_INT3_PER_CHANNEL_CONFIG = QuantizationConfig(weight=INT3_PER_CHANNEL_ASYM_SPEC)
W_INT3_PER_GROUP_CONFIG = QuantizationConfig(weight=INT3_PER_GROUP_ASYM_SPEC)
W_INT2_PER_GROUP_CONFIG = QuantizationConfig(weight=INT2_PER_GROUP_ASYM_SPEC)


QUANT_SCHEME_TO_CONFIG = {
    "no_quant": QuantizationConfig(),
    ...
    "w_fp8_per_channel_static_a_fp8_per_token_dynamic": W_FP8_PER_CHANNEL_STATIC_A_FP8_PER_TOKEN_DYNAMIC_CONFIG,
    ...
    "w_int3_per_channel_asym": W_INT3_PER_CHANNEL_CONFIG,
}
...
def get_global_config(
    ...
    if quant_scheme in QUANT_SCHEME_TO_CONFIG:
        ...
        global_quant_config = QuantizationConfig(
            input_tensors=OCP_MXFP8_E4M3_SPEC(scale_calculation_mode, True),
            weight=OCP_MXFP4_SPEC(scale_calculation_mode, False),
        )
        ...
        global_quant_config.weight.set_group_size(group_size)
    elif quant_scheme == "w_int3_per_group_asym":
        global_quant_config = W_INT3_PER_GROUP_CONFIG
        global_quant_config.weight.set_group_size(group_size)
