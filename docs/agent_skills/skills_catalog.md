...
| `quark-torch-file2file-quantization` | L1 | Low-memory file2file quantization for very large safetensors LLMs that cannot be loaded whole; supports external `LLMTemplate` registration. | "run file2file quantization", "quantize without loading the model", "file2file for DeepSeek/Qwen/MoE" |
| `quark-torch-ptq` | L2 | End-to-end Torch LLM PTQ pipeline: intake → plan → script → optional execution. | "quantize my model", "run PTQ end to end", "quantize Llama/Qwen/Mistral with FP8/INT4" |
