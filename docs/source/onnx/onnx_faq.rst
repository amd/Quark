...
This error is caused by the input model size exceeding 2GB. Set ``use_external_data_format=True`` and ``OptimizeModel=False`` in kwargs.
...
This error is caused by the calibration data is NCHW and the shape of model input is NHWC. Set ``ConvertNchwToNhwc=True`` in kwargs. For more detailed information, see :doc:`Tools <tools>`.
