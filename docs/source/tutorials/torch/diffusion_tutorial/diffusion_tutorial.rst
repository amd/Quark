...
``HF_HOME`` as the location to cache downloaded models and datasets,
respectively. You can set these environment variables to specify your
own paths. By default they will be under ``~/.cache/huggingface/``.
...
.. code:: ipython3
    ...
    DEFAULT_W_FP8_PER_TENSOR_CONFIG = QLayerConfig(weight=DEFAULT_FP88_PER_TENSOR_SYM_SPEC)
    quant_config = QConfig(global_quant_config=DEFAULT_W_FP8_PER_TENSOR_CONFIG)
...
To quantize accurately, Quark needs to observe the inputs the U-Net
actually sees during a pipeline run (the noisy latents, the timestep,
and the text-conditioning embeddings). Rather than building these
tensors by hand, Quark provides ``get_calib_dataloader``, which hooks
into the pipeline, runs it over a handful of calibration prompts, and
captures the U-Net’s inputs automatically. Each captured sample is a
dictionary keyed by the U-Net’s ``forward`` parameter names, which
``ModelQuantizer.quantize_model`` consumes directly.

We can use a few hand-written prompts for calibration – no gated
datasets or Hugging Face login required. More prompts and more denoising
steps give more calibration samples (``len(prompts) * n_steps`` total);
a handful is enough for this tutorial.

Collect calibration data by running the pipeline over a few prompts:
...
.. code:: ipython3
    ...
    calib_dataloader = get_calib_dataloader(pipe, pipe.unet, calib_prompts, n_steps=20, guidance_scale=7.5)
...
.. code:: ipython3
    ...
    quant_model = quantizer.quantize_model(pipe.unet, calib_dataloader)
...
.. code:: ipython3
    ...
    frozen_quantized_model = quantizer.freeze(quant_model)
    ...
    export_onnx(frozen_quantized_model, export_path, input_args)
