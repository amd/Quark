...
the ONNX quantizer of Quark. Layerwise percentile calibration offers a
more granular approach than standard percentile calibration. Instead of
using global min/max values or a single percentile for the whole model,
it collects weight and/or activation statistics for each layer and
selects clipping thresholds from those per-layer distributions. This
produces more appropriate quantization ranges that better reflect each
layer’s unique dynamic range.
...
then, it changes to the layerwise procedure to reduce accuracy
...
.. code:: ipython3
    ...
    from quark.onnx import CalibMethod, Int8Spec, ModelQuantizer, QConfig, QLayerConfig
    ...
    def quantize_model(args: dict) -> None:
        ...
        if args["calib_method"] == "minmax":
            ...
            activation_spec.set_calibration_method(CalibMethod.LayerwisePercentile)
...
.. code:: ipython3
    ...
    quant_config = {
        ...
        "output_model_path": "models/resnet152_minmax_quantized.onnx",
        "calib_method": "minmax",
...
The cell applies the same default quantization scheme but uses layerwise
option, then generates the quantized model into the models directory.

.. code:: ipython3

    quant_config_with_layerwise = copy.deepcopy(quant_config)
    quant_config_with_layerwise["output_model_path"] = "models/resnet152_layerwise_quantized.onnx"
    quant_config_with_layerwise["calib_method"] = "layerwise_percentile"
    
    quantize_model(quant_config_with_layerwise)
...
Then, specify the path to the quantized model **with MinMax** and record
its accuracy on ImageNet val dataset

.. code:: ipython3

    quant_eval_config = copy.deepcopy(eval_config)
    quant_eval_config["onnx_input"] = "models/resnet152_minmax_quantized.onnx"
    
    evaluate(quant_eval_config)

Last, specify the path to the quantized model **with Layerwise** and
record its accuracy on ImageNet val dataset

.. code:: ipython3

    layerwise_eval_config = copy.deepcopy(eval_config)
    layerwise_eval_config["onnx_input"] = "models/resnet152_layerwise_quantized.onnx"
    
    top1, top5 = evaluate(layerwise_eval_config)
...
.. code:: ipython3
    ...
    model_name = "layerwise"
    
    benchmarks = {"layerwise": {"acc1": 71.3, "acc5": 90.3}}
            ...
            "data": [
                {
                    "experiment": {
                        ...
                        "settings": {
                            ...
                            "quantization_algorithm": "Layerwise Percentile",
                        },
                    },
                    "metrics": [
                        {
