...
AdaRound, short for “Adaptive Rounding,” is an advanced quantization
...
AdaRound uses an adaptive approach to determine the optimal rounding of
weights.
...
- Quantizatize without AdaRound

- Quantizatize with AdaRound
...
also enables the AdaRound procedure to reduce accuracy degradation
during quantization.
...
.. code:: ipython3
    ...
    from quark.onnx import AdaRoundConfig, CalibMethod, Int8Spec, ModelQuantizer, QConfig, QLayerConfig
...
The cell defines a quantization config with AdaRound disabled, and then
...
AdaRound option, then generates the quantized model into the models
directory.

.. code:: ipython3

    quant_config_with_adaround = copy.deepcopy(quant_config)
    quant_config_with_adaround["output_model_path"] = "models/mobilenetv2_050.lamb_in1k_adaround_quantized.onnx"
    quant_config_with_adaround["use_adaround"] = True
    
    quantize_model(quant_config_with_adaround)

5) Evaluation and Expected Results
----------------------------------

Evaluation is performed on the ImageNet validation set. We compare three
models — (1) full-precision, (2) quantized without AdaRound, and (3)
quantized with AdaRound — to assess AdaRound’s effectiveness. The
...
Then, specify the path to the quantized model **without AdaRound** and
...
Last, specify the path to the quantized model **with AdaRound** and
record its accuracy on ImageNet val dataset

.. code:: ipython3

    adaround_eval_config = copy.deepcopy(eval_config)
    adaround_eval_config["onnx_input"] = "models/mobilenetv2_050.lamb_in1k_adaround_quantized.onnx"
    
    top1, top5 = evaluate(adaround_eval_config)
...
|        | Model    | AdaRound                  | AdaRound                |
+========+==========+===========================+=========================+
| Model  | 8.4 MB   | 2.3 MB                    | 2.4 MB                  |
| Size   |          |                           |                         |
+--------+----------+---------------------------+-------------------------+
| Prec@1 | 65.424 % | 1.708 %                   | 39.790 %                |
+--------+----------+---------------------------+-------------------------+
| Prec@5 | 85.788 % | 5.690 %                   | 63.004 %                |
+--------+----------+---------------------------+-------------------------+
...
.. code:: ipython3
    ...
    benchmarks = {"adaround": {"acc1": 39.790, "acc5": 63.004}}
            ...
            "data": [
                {
                    "experiment": {
                        ...
                        "settings": {
                            ...
                            "quantization_algorithm": "AdaRound",
                        },
                    },
                    "metrics": [
                        {
